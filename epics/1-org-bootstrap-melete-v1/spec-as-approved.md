# Melete — the design as approved, 2026-08-09

**This file is immutable. It must never be edited.**

Below is the melete v1 design exactly as it was approved on 2026-08-09, before
any implementation work began. It is a snapshot of a moment, not a description
of the tool. Wherever the design has since changed, this file is out of date on
purpose, and correcting it would destroy the only thing it is for.

The living document is [`spec.md`](spec.md) in this directory. It carries this
text plus every amendment made since approval — including decisions 14–26, none
of which appear here. Any question about what melete *is* is answered there;
this file answers only what it was.

**Why it exists separately.** Pull requests in this organization squash-merge,
so the very first commit of `spec.md` already contained the amendments made
during that pull request's review. The as-approved text was never committed on
its own and survives nowhere else in git history — it existed only as a loose
file on the author's disk, and one deletion away from being gone.

**Why it is kept.** The epic's retrospective asks two questions that cannot be
answered without it. §0 is *what we set out to do → what shipped*, and §1 is
*how the plan evolved*. Both are differences, and a difference needs both terms.
The code and the merged pull requests supply what shipped; only this file
supplies what was approved.
...
1. **`mnemosys-project/.github`** — epic home, label registry, org metadata,
   shared authorization and secrets context. Modeled on `vergil-project/.github`,
   cross-checked against `logical-minds-foundry`.
...
**Status:** Design approved 2026-08-09. Pending interactive review before
`vergil:epic-create`.
